...
export const presentEventViewingDialog = (options) => {
  const toCall = () =>
    AddCalendarEvent.presentEventViewingDialog(processColorsIOS(options));
  return withPermissionsCheck(false, toCall);
};

export const presentEventEditingDialog = (options) => {
  const toCall = () =>
    AddCalendarEvent.presentEventEditingDialog(processColorsIOS(options));
  return withPermissionsCheck(false, toCall);
};
...
const withPermissionsCheckIOS = async (
  writeOnly,
  toCallWhenPermissionGranted
) => {
